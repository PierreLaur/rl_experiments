...
def evaluate(env, policy, tries, render=False, display=True):
    ...
    for i in range(tries):
        ...
        while not done and step < 100:
            state, reward, done, _ = env.step(np.random.choice(
                range(env.action_space.n), p=policy[state]))
            if render and i == tries-1:
                env.render()
            ret += reward
            step += 1
        score += ret
    score /= tries
    if display:
        print("Average score over {} tries : {}".format(tries, score))
    return score
...
def on_policy_mc_control(env, num_episodes, gamma, epsilon, episode_maxlength=10000, init_policy=None, low_reward_early_stopping=False, plot_return=False, plot_color='r', plot_label=None, xlims=(0, 10000), ylims=(-100, 100), EWMA_factor=0):
    ...
    if plot_return:
        axes, line, xdata, ydata = setup_plot(
            plot_color, plot_label, xlims, ylims)
    ...
    with Bar('Sampling for On-Policy MC...', max=num_episodes) as bar:
        for episode in range(num_episodes):
            ...
            G = 0
            temp_returns = {}
            for state,action,reward,_ in reversed(Samples):
                G = gamma*G + reward
                temp_returns[(state,action)] = G

            for ((state,action),ret) in temp_returns.items():
                Returns[state][action][1] = ret
                Returns[state][action][0] = Returns[state][action][0]+1
                Q[state][action] = Returns[state][action][1] / \
                    Returns[state][action][0]

                # Derive an epsilon-greedy policy from the q-value
                best_action = np.argmax(Q[state], 0)
                for a in range(nA):
                    if a == best_action:
                        pi[state][a] = 1-current_epsilon+(current_epsilon/nA)
                    else:
                        pi[state][a] = (current_epsilon/nA)
        ...
        for episode in range(num_episodes):
            ...
            while not done and steps < episode_maxlength:
                ...
                new_action = np.random.choice(
                    nA, p=derive_greedy(Q[state], epsilon))
                Q[state][action] += step_size * \
                    (reward + discount*Q[new_state]
        ...
        for episode in range(num_episodes):
            ...
            while not done and steps < episode_maxlength:
                ...
                action = np.random.choice(
                    nA, p=derive_greedy(Q[state], current_epsilon))
                new_state, reward, done, prob = env.step(action)
                Q[state][action] += current_step_size * \
            ...
            if plot_return:
                if episode % 100 == 0:
                    ...
                    for s in range(nS):
                        pi[s][np.argmax(Q[s])] = 1
                    ev = plot(env, pi, line, xdata,
                              ydata, episode, EWMA_factor)
...
def double_q_learning(env, num_episodes, discount, step_size, epsilon, stepsize_decay_rate=0, episode_maxlength=10000, init_Q=None, show_Qvalues=False, plot_return=False, plot_color='r', plot_label=None, xlims=(0, 10000), ylims=(-100, 100), EWMA_factor=0, optimal_return=None):
    ...
    with Bar('Running Double Q-Learning...', max=num_episodes) as bar:
        for episode in range(num_episodes):
            ...
            while not done and steps < episode_maxlength:
                action = np.random.choice(
                    nA, p=derive_greedy(Q1[state]+Q2[state], epsilon))
                new_state, reward, done, prob = env.step(action)

                if np.random.random() < 0.5:
                    Q1[state][action] += step_size * \
                        (reward + discount *
                         Q2[new_state][np.argmax(Q1[new_state])] - Q1[state][action])
                else:
                    Q2[state][action] += step_size * \
                        (reward + discount *
                         Q1[new_state][np.argmax(Q2[new_state])] - Q2[state][action])
                state = new_state
                steps += 1
    ...
    with Bar('Running Expected SARSA...', max=num_episodes, redirect_stdout=True) as bar:
        for episode in range(num_episodes):
            ...
            while not done and steps < episode_maxlength:
                ...
                Q[state][action] += step_size * \
                    (reward + discount*exp - Q[state][action])
    ...
    with Bar('Running Dyna-Q...', max=num_episodes) as bar:
        for episode in range(num_episodes):
            ...
            while not done and step < episode_maxlength:
                action = np.random.choice(
                    range(nA), p=derive_greedy(Q[state], epsilon))
                ...
                for i in range(model_queries):
                    (sim_state, sim_action) = visited[np.random.choice(
                        range(len(visited)))]
                    (sim_reward, sim_new_state) = Model[sim_state][sim_action]
                    Q[sim_state][sim_action] += step_size * \
                        (sim_reward+discount *
                         max(Q[sim_new_state])-Q[sim_state][sim_action])
                state = new_state
                step += 1
    ...
    with Bar('Sampling for Off-Policy MC...', max=num_episodes) as bar:
        for episode in range(num_episodes):
            ...
            for sample in reversed(Samples):
                ...
                # Update Q
                Q[sample[0]][sample[1]
                             ] += (W/C[sample[0]][sample[1]])*(G-Q[sample[0]][sample[1]])
                ...
                for act in range(len(pi[sample[0]])):
                    if act == np.argmax(Q[sample[0]]):
                        pi[sample[0]][act] = 1-epsilon * \
                            (1-((episode+1)/num_episodes))
                    else:
                        pi[sample[0]][act] = epsilon * \
                            (1-((episode+1)/num_episodes))/(nA-1)
    ...
    with Bar(max=num_episodes) as bar:
        for episode in range(num_episodes):
            ...
            while not done and step < episode_maxlength:
                ...
                update_queue(P, state, action, reward,
                             new_state, queue_threshold)
